...
# Обновить ссылки в письменной форме
        citations_data = [
            {
                'id': paper.id,
                'title': paper.title,
...
markdown += f"**Время создания**: {writing.created_at.strftime('%Y-%m-%d %H:%M:%S')}\n\n"
...
"""Экспорт в формат LaTeX"""
        ...
        if writing.content:
# Простое преобразование Markdown в LaTeX
...
"""Экспорт в формат документа Word"""
#Здесь вы можете использовать библиотеку python-docx
...
# Генерация шаблонов схем на основе типа письма
...
{"title": "Методы исследования", "уровень": 1, "content": "План эксперимента и анализ данных"},
                {"title": "主要结果", "level": 1, "content": "关键发现和数据分析"},
{"title": "Вывод и значение", "уровень": 1, "content": "Вклад и влияние в исследованиях"}
            ]
        elif writing_type == "critique":
            outline = [
                {"title": "论文概述", "level": 1, "content": "研究内容总结"},
                {"title": "优点分析", "level": 1, "content": "研究的创新性和贡献"},
{"title": "Проблемы и ограничения", "уровень": 1, "content": "Проблемы в исследовании"},
